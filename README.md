...
- Inbound Telnyx SMS/MMS, Twilio SMS/MMS, and Telnyx fax storage, including retrying background media ingestion, lazy inline PDF fax viewing, and background conversion of cached 3GP videos when `ffmpeg` is installed.
...
- Separate action-triage assistant API for reading unread context and maintaining isolated review state without messaging authority.

## Programmatic messaging API

Set a dedicated API token and restart Switchboard. Keep this token secret; it
can send messages using your configured provider accounts and is separate from
browser sign-in.

```env
TEXTING_API_TOKEN=replace-with-a-long-random-token
```

Generate one with `python -m secrets token_urlsafe 32`. Send an SMS or MMS with:

```bash
curl -X POST https://switchboard.example/api/v1/messages \
  -H "Authorization: Bearer $TEXTING_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "from_number": "+15551230001",
    "to_numbers": ["+15551230002"],
    "text": "Sent by another program",
    "media_urls": []
  }'
```

A successful provider submission returns HTTP `201`, the Switchboard message,
and a `status_url`. The message is stored in the same conversation database and
therefore appears in the web interface. Poll the returned URL to observe status
callbacks such as `queued`, `sent`, `delivered`, or `delivery_failed`:

```bash
curl https://switchboard.example/api/v1/messages/123 \
  -H "Authorization: Bearer $TEXTING_API_TOKEN"
```

The response includes `accepted` (the provider did not reject the submission),
`delivered` (a delivery callback has confirmed it), provider identifiers, and
timestamps. The messaging API bearer token is accepted only under `/api/v1/`;
it does not grant access to Switchboard settings or other browser APIs.
Provider callback URLs must remain configured for delivery state to progress
after submission.

## Action-triage assistant API

The assistant API is a separate, read-mostly surface for tools that turn unread
threads into action choices. Configure a unique token and restart Switchboard:

```env
SWITCHBOARD_ASSISTANT_API_TOKEN=replace-with-a-different-long-random-token
```

Generate it with `python -m secrets token_urlsafe 32`. It must differ from
`TEXTING_API_TOKEN`; Switchboard disables both programmatic scopes if the two
configured values match. The assistant token is accepted only under
`/api/assistant/v1/` and cannot send or schedule messages, change contacts,
mark conversations read, or hide conversations.

List unread conversations using compact triage summaries:

```bash
curl 'https://switchboard.example/api/assistant/v1/unread-conversations?limit=50&only_new_since_last_review=true' \
  -H "Authorization: Bearer $SWITCHBOARD_ASSISTANT_API_TOKEN"
```

Optional query parameters are `personal_number`, `include_hidden` (default
`false`), and `only_new_since_last_review` (default `true`). The result includes
the latest unread message, unread count, contact and identity phone numbers,
media/voicemail flags, queued-message presence, and the latest assistant review.
The limit defaults to 50 and is capped at 200.

Fetch recent context only for a conversation that needs deeper analysis:

```bash
curl 'https://switchboard.example/api/assistant/v1/conversations/142/context?message_limit=20' \
  -H "Authorization: Bearer $SWITCHBOARD_ASSISTANT_API_TOKEN"
```

Context includes bounded message and attachment metadata, participant contact
details, and queued outgoing messages. It does not expose attachment paths,
provider payloads, or contact-provider raw data. `message_limit` defaults to 20
and is capped at 100.

Record the review state through the message that triggered the action item:

```bash
curl -X POST https://switchboard.example/api/assistant/v1/action-reviews \
  -H "Authorization: Bearer $SWITCHBOARD_ASSISTANT_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "conversation_id": 142,
    "through_message_id": 837,
    "state": "deferred",
    "defer_until": "2026-08-03T09:00:00-04:00",
    "analysis_version": "triage-v1"
  }'
```

Valid states are `presented`, `dismissed`, `deferred`, and `resolved`.
`defer_until` is required for `deferred` and must include a timezone offset.
The review is upserted by conversation and triggering message. A later inbound
message reopens the conversation automatically, while a deferred item becomes
eligible again after its deferral expires. Review writes never alter the normal
Switchboard unread or hidden state.

List the latest unresolved (`presented` or `deferred`) review per conversation:

```bash
curl https://switchboard.example/api/assistant/v1/action-reviews/unresolved \
  -H "Authorization: Bearer $SWITCHBOARD_ASSISTANT_API_TOKEN"
```

Each result identifies whether a newer inbound message arrived and whether a
deferral expired. This endpoint accepts an optional `limit` capped at 200.
...
The image includes `ffmpeg` for browser-friendly video conversion. Switchboard serves outbound upload staging itself at `/uploads/<random-file>`, so Docker installs do not need a second public static directory. The Docker health check uses `/api/health`.
...
Administrators can add limited accounts from `Settings` > `Limited users`. Each account is assigned one active sender number and can see and send only messages involving that number, including when a remote contact also has conversations with another Switchboard number. Read and unread state is tracked independently for every account: administrator actions never mark a limited user's messages read, and limited users sharing a sender number do not share read state. A limited account starts with a private snapshot of names already attached to its number's conversations and can add or rename only contacts from those conversations; later contact changes made by administrators or other limited accounts are not exposed. Limited accounts cannot open global settings, security, statistics, database downloads, contact sync, or number-management routes. Their Settings button opens a preferences-only view for choosing a theme family and light/dark mode. Changing an account's username, password, assigned number, or active state invalidates its existing sessions; assigning a different number also replaces its contact snapshot with the contacts for that number and resets that account's read state.
...
- `TEXTING_API_TOKEN`: dedicated bearer token for the versioned programmatic messaging API.
- `SWITCHBOARD_ASSISTANT_API_TOKEN`: separate bearer token for the read-mostly action-triage API; it must differ from `TEXTING_API_TOKEN`.
- `TEXTING_ALLOW_UNSIGNED_PROVIDER_WEBHOOKS`: unsafe compatibility switch for isolated local testing. Production Telnyx and Twilio callbacks are rejected when their signing credential is missing.
...
- `TEXTING_MEDIA_DIR`: local directory for received media, cached attachments, converted videos, fax PDFs, and voicemail recordings.
...
- `TELNYX_PUBLIC_KEY`: required for Telnyx webhook signature verification outside explicit local unsigned-webhook testing.
- `TELNYX_API_BASE`: Telnyx API base URL; normally leave the default.
- `TWILIO_ACCOUNT_SID`: required for sending through Twilio.
- `TWILIO_AUTH_TOKEN`: required for Twilio sends and webhook signature verification outside explicit local unsigned-webhook testing.
...
- `TEXTING_CACHE_ATTACHMENTS`: legacy remote-cache preference retained for configuration compatibility. Provider-received media is always stored locally by the background ingestion worker.
...
Restarting the server seeds any new `TEXTING_PERSONAL_NUMBERS` into the local sender identity table. Existing deployments can keep using that deployment-managed scheme. You can also use **Settings > Numbers** or **Numbers > Add number** in the sidebar to add a sender directly to SQLite and choose its Telnyx or Twilio provider without editing `.env` or restarting. UI-added and `.env`-seeded numbers coexist, and adding a provider override preserves the effective legacy `TEXTING_PROVIDER_BY_NUMBER` map. The Numbers panel can rename and recolor sender identities, and can enable a vacation auto-reply for one sender number at a time. Auto-replies are sent only for direct inbound texts, use the selected sender number's configured Telnyx/Twilio provider, and are rate-limited per recipient by the cooldown on that number. Settings > Messaging can adjust the default provider and per-number provider map without editing source.
...
Use this endpoint for both Messaging Profile webhooks and Programmable Fax Application webhooks. Inbound `fax.received` events are stored immediately, then the PDF is downloaded by the attachment worker and shown in a fixed-height, lazy inline viewer. Keeping one PDF attachment avoids a large thread reflow for multi-page faxes. Failed inbound fax attempts are stored too, including Telnyx's `failure_reason` when one is provided. Outbound fax requires `TELNYX_FAX_CONNECTION_ID`; long-press the composer attachment button to open the fax sender.

Some inbound phone videos arrive as `.3gp` / `video/3gpp`, which most browsers cannot play inline. When `ffmpeg` is installed, the attachment worker converts received 3GP media to browser-friendly MP4 before marking it ready, then stores the converted attachment path for future views:
...
Twilio voice callbacks use `TWILIO_AUTH_TOKEN` for signature verification. Telnyx voice callbacks use `TELNYX_PUBLIC_KEY` for signature verification. Missing signing credentials reject provider callbacks by default; isolated local tests can explicitly set `TEXTING_ALLOW_UNSIGNED_PROVIDER_WEBHOOKS=1`.
...
Received provider attachments are committed as metadata immediately, then downloaded into `TEXTING_MEDIA_DIR` by a background worker. Failed downloads retry with backoff, and 3GP conversion also runs outside the request path. Inbound downloads require public HTTPS destinations and reject local/private resolved addresses, including after redirects. The worker connects directly and does not use `HTTP_PROXY`/`HTTPS_PROXY`, so hosts that require an outbound proxy need a direct egress exception for provider media. Threads therefore open without waiting on provider media or conversion work.
